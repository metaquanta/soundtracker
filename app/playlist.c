...
playlist_draw_contents(Playlist* p)
{
    ...
    for (i = 0; i <= 4; i++) {
        if ((i >= 2 - p->current_position) && (i <= p->length - p->current_position + 1)) {
            thing = p->numlabels[i];
...
void playlist_set_nth_pattern(Playlist* p,
    int pos,
    int pat)
{
...
int playlist_get_nth_pattern(Playlist* p,
    int pos)
{
    g_assert(pos >= 0 && pos < p->length);
...
void playlist_set_position(Playlist* p,
    int pos)
{
    g_assert(pos >= 0 && pos < p->length);
...
void playlist_set_restart_position(Playlist* p,
    int pos)
{
    g_assert(pos >= 0 && pos < p->length);
...
playlist_class_init(PlaylistClass* class)
{
    ...
    playlist_signals[SIG_RESTART_POSITION_CHANGED] = g_signal_new("restart_position_changed",
        G_TYPE_FROM_CLASS(g_object_class),
        (GSignalFlags)G_SIGNAL_RUN_FIRST,
        G_STRUCT_OFFSET(PlaylistClass, restart_position_changed),
        NULL, NULL,
        gtk_marshal_NONE__INT, G_TYPE_NONE, 1, G_TYPE_INT);
    playlist_signals[SIG_SONG_LENGTH_CHANGED] = g_signal_new("song_length_changed",
        G_TYPE_FROM_CLASS(g_object_class),
        (GSignalFlags)G_SIGNAL_RUN_FIRST,
