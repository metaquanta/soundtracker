...
void track_editor_do_the_note_key(int notekeymeaning,
    gboolean pressed,
    guint32 xkeysym,
    int modifiers,
    gboolean always_poly)
{
    ...
    if (pressed) {
        if (note_is_running(note) == -1) {
            if (gui_settings.try_polyphony && (always_poly || (!GTK_TOGGLE_BUTTON(editing_toggle)->active)) && gui_playing_mode != PLAYING_SONG && gui_playing_mode != PLAYING_PATTERN) {
