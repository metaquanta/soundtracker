...
void sample_display_set_mixer_position(SampleDisplay* s,
    int offset)
{
    g_return_if_fail(s != NULL);
    g_return_if_fail(IS_SAMPLE_DISPLAY(s));
...
void sample_display_set_window(SampleDisplay* s,
    int start,
    int end)
{
    g_return_if_fail(s != NULL);
...
sample_display_realize(GtkWidget* widget)
{
    ...
    gdk_gc_set_foreground(s->bg_gc, &SAMPLE_DISPLAY_GET_CLASS(SAMPLE_DISPLAY(widget))->colors[SAMPLE_DISPLAYCOL_BG]);
    gdk_gc_set_foreground(s->fg_gc, &SAMPLE_DISPLAY_GET_CLASS(SAMPLE_DISPLAY(widget))->colors[SAMPLE_DISPLAYCOL_FG]);
    gdk_gc_set_foreground(s->zeroline_gc, &SAMPLE_DISPLAY_GET_CLASS(SAMPLE_DISPLAY(widget))->colors[SAMPLE_DISPLAYCOL_ZERO]);
    if (s->edit) {
        s->loop_gc = gdk_gc_new(widget->window);
        s->mixerpos_gc = gdk_gc_new(widget->window);
        gdk_gc_set_foreground(s->loop_gc, &SAMPLE_DISPLAY_GET_CLASS(SAMPLE_DISPLAY(widget))->colors[SAMPLE_DISPLAYCOL_LOOP]);
        gdk_gc_set_foreground(s->mixerpos_gc, &SAMPLE_DISPLAY_GET_CLASS(SAMPLE_DISPLAY(widget))->colors[SAMPLE_DISPLAYCOL_MIXERPOS]);
    }
    ...
    int width)
{
    ...
    case ST_MIXER_FORMAT_S16_LE | ST_MIXER_FORMAT_STEREO:
        c = (((gint16*)s->data)[OFFSET_RANGE(s->datalen, XPOS_TO_OFFSET(x - 1))] + ((gint16*)s->data)[OFFSET_RANGE(s->datalen, XPOS_TO_OFFSET(x - 1) + 1)]) >> 1;

        while (width >= 0) {
            d = (((gint16*)s->data)[OFFSET_RANGE(s->datalen, XPOS_TO_OFFSET(x))] + ((gint16*)s->data)[OFFSET_RANGE(s->datalen, XPOS_TO_OFFSET(x) + 1)]) >> 1;
            gdk_draw_line(win, gc,
                x - 1, ((32767 - c) * sh) >> 16,
                x, ((32767 - d) * sh) >> 16);
            c = d;
    ...
    case ST_MIXER_FORMAT_S8 | ST_MIXER_FORMAT_STEREO:
        c = (((gint8*)s->data)[OFFSET_RANGE(s->datalen, XPOS_TO_OFFSET(x - 1))] + ((gint8*)s->data)[OFFSET_RANGE(s->datalen, XPOS_TO_OFFSET(x - 1) + 1)]) >> 1;

        while (width >= 0) {
            d = (((gint8*)s->data)[OFFSET_RANGE(s->datalen, XPOS_TO_OFFSET(x))] + ((gint8*)s->data)[OFFSET_RANGE(s->datalen, XPOS_TO_OFFSET(x) + 1)]) >> 1;
            gdk_draw_line(win, gc,
                x - 1, ((127 - c) * sh) >> 8,
                x, ((127 - d) * sh) >> 8);
            c = d;
