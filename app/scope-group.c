...
void scope_group_set_num_channels(ScopeGroup* s,
    int num_channels)
{
    int i;
    ...
    for (i = 0; i < 32; i++) {
        ...
        if (s->scopes_on)
            gtk_widget_show(GTK_WIDGET(s->scopes[i]));
#endif
    }
...
void scope_group_enable_scopes(ScopeGroup* s,
    int enable)
{
    int i;
...
gint scope_group_timeout(ScopeGroup* s)
{
    double time1, time2;
    int i, l;
...
void scope_group_set_update_freq(ScopeGroup* s,
    int freq)
{
    s->update_freq = freq;
    if (s->scopes_on && s->gtktimer != -1) {
            ...
            if (sg->on_mask == 0xFFFFFFFF) {
                for (i = 0; i < sg->numchan; i++)
                    if (sg->scopebuttons[i] != t)
                        ...
                        sg->on_mask = (1 << i) | (0xFFFFFFFF << sg->numchan);
            } else {
                for (i = 0; i < sg->numchan; i++)
